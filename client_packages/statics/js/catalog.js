...
const vehicleClasses = [
    // Cars
    [ 
        {'id': 0, 'desc': 'cardealer.compacts'}, {'id': 3, 'desc': 'cardealer.coupes'}, {'id': 6, 'desc': 'cardealer.sports'}, 
        {'id': 4, 'desc': 'cardealer.muscle'}, {'id': 2, 'desc': 'cardealer.suv'}, {'id': 1, 'desc': 'cardealer.sedans'}, 
        {'id': 9, 'desc': 'cardealer.off-road'}
    ],
    // Motorbikes
    [ {'id': 8, 'desc': 'cardealer.motorcycles'}, {'id': 9, 'desc': 'cardealer.off-road'} ]
];
...
function initializeCatalog(dealership, vehicleJSON) {
    ...
    let classes = vehicleClasses[dealer].map(function(type) { return type.desc; });
    populateSelectBox('multi-select', classes, true);
...
function populateSelectBox(id, elementArray, checked) {
    ...
    for (let i = 0; i < elementArray.length; i++) {
        ...
        if (checked) {
            ...
            // Add the classes
            checkElement.classList = 'check';
            checkElement.setAttribute('data-type', vehicleClasses[dealer][i].id);
